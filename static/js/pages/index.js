...
function redrawHistory(response) {
    ...
    if (response.certifications.length > 0) {
        ...
        $.each(response.certifications, function (index) {
            ...
            var row = '<tr><td><span class="check"></span><a href="https://etherscan.io/tx/' + transactionHash + '" aria-label="Transaction ' + transactionHash + '" target="_blank">' + transactionHash + '</a></td><td>' + transactionCreated + '</td></tr>';
    ...
    if (response.ongoing_submissions.length > 0) {
        ...
        $.each(response.ongoing_submissions, function (index) {
            ...
            var row = '<tr><td><a href="https://etherscan.io/tx/' + transactionHash + '" aria-label="Transaction ' + transactionHash + '" target="_blank">' + transactionHash + '</a></td><td>' + transactionCreated + '</td></tr>';
